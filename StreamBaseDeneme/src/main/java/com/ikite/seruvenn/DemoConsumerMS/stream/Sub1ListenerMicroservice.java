...
@Component
public class Sub1ListenerMicroservice extends StreamBase {
	
	private static final Logger LOG = LoggerFactory.getLogger(Sub1ListenerMicroservice.class);

	@Autowired
	IEventBuilderService eventBuilderService;

	@Autowired
	public Sub1ListenerMicroservice(Environment env) {
		super(env);
	}
	...
	public KStream<String, Message> buildForwardStream() {
		...
		return forwardCommandStream.map((key, command) -> {
			...
			try {
				return eventBuilderService.buildNewStreamEvent(key, EventType.SUB1_SUCCESSFUL.toString(), command.getSagaCode(),
						new Foo());
			} catch (Exception e) {
	...
	public KStream<String, Message> buildReverseStream() {
		...
		return reverseCommandStream.map((key, command) -> {
			...
			try {
				return eventBuilderService.buildNewStreamEvent(key, EventType.SUB1_ROLLBACK_OK.toString(), command.getSagaCode(),
						new Foo());
			} catch (Exception e) {
				LOG.error(e.getMessage());
				return null;
